...
use self::worldgen::WorldgenMenu;
...
impl Page {
	pub fn all() -> &'static [Page] {
		&[Page::Worldgen, Page::Simulation, Page::Statistics]
	}
}
			...
			.show(ctx, |ui| {
				...
				ui.horizontal_wrapped(|ui| {
					for page in Page::all() {
						ui.selectable_value(&mut self.page, *page, format!("{:?}", page));
					}
				});
				ui.separator();
				...
				ScrollArea::vertical()
					.show(ui, |ui| {
						if self.page == Page::Worldgen {
							self.worldgen.show(ui);
						} else if self.page == Page::Simulation {
							ui.add_space(5.0);
